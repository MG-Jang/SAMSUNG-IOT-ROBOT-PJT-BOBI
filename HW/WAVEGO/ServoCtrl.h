...
void pitchYawRollHeightCtrl_Testver(float pitchInput, float yawInput, float rollInput, float heightInput){
  legPosBuffer[1]  = STAND_HEIGHT + pitchInput + rollInput + heightInput;
  legPosBuffer[4]  = STAND_HEIGHT - pitchInput + rollInput + heightInput;
  legPosBuffer[7]  = STAND_HEIGHT + pitchInput - rollInput + heightInput;
  legPosBuffer[10] = STAND_HEIGHT - pitchInput - rollInput + heightInput;
  ...
  for (float i = 0; i<=1; i+=0.02){
    singleLegCtrl(1,  WALK_EXTENDED_X, legPosBuffer[1] /50*i , legPosBuffer[2]/50*i);
    singleLegCtrl(2, -WALK_EXTENDED_X, legPosBuffer[4] /50*i, legPosBuffer[5]/50*i);
    singleLegCtrl(3,  WALK_EXTENDED_X, legPosBuffer[7] /50*i, legPosBuffer[8]/50*i);
    singleLegCtrl(4, -WALK_EXTENDED_X, legPosBuffer[10] /50*i, legPosBuffer[11]/50*i);
  }
}
...
// unsigned long past_test = 0;
// int flag_test = 0;
void functionOk(){
  pitchYawRollHeightCtrl(6, 0, 0, 0);
  //고개 끄덕이기
  /*pitchYawRollHeightCtrl(2 * 3, gestureLR, 0, 0);
  delay(200);
  pitchYawRollHeightCtrl(-2 * 3, gestureLR, 0, 0);
  delay(100);
  pitchYawRollHeightCtrl(2 * 3, gestureLR, 0, 0);
  delay(100);
  pitchYawRollHeightCtrl(0, gestureLR, 0, 0);
*/
  //functionActionA();
  //functionActionB();
  /*for (int i = 0; i < 10; i++){
...
/*
  for (int i = 0; i < 10; i++){
    ...
    unsigned long now_test = millis();
    Serial.print(i);
    Serial.print(" in for loop");
    Serial.print(past_test);
    Serial.println(now_test);
    if (now_test - past_test >= 50){
      past_test = now_test;
      flag_test = 1;
    }

    if (flag_test == 1){
      Serial.print(gestureUD);
      Serial.println("move");
      gestureUD -= 2;
      if(gestureUD <-gestureOffSetMax){gestureUD =-gestureOffSetMax;}
      pitchYawRollHeightCtrl(gestureUD, gestureLR, 0, 0);
...
void robotCtrl(){
  ...
  // function ctrl.
    ...
    setSingleLED(0,matrix.Color(0, 128, 255));// blue
